...
Log c_debug;
#define LOG(x) c_debug.println(x)
#define LOG_INT(x) c_debug.printint(x)
...
string Contract::SetupContractData(const string *func, ...) {
    ...
    for (int i = 0; i < paramCount; ++i) {
        if (strncmp(params[i].c_str(), "string", sizeof("string")) == 0) {
            string output = GenerateBytesForString(va_arg(args, string*));
            dynamicRet = dynamicRet + string(output);
            ...
            else if (strncmp(params[i].c_str(), "int", sizeof("int")-1) == 0 || strncmp(params[i].c_str(), "bool", sizeof("bool")) == 0) {
    ...
    int offset = strCount + noStrCount;
    string strOffsets = "";
    ...
    for (int i = 0; i < paramCount; ++i) {
        if (strncmp(params[i].c_str(), "string", sizeof("string")) == 0) {
            ...
            char hexOffset[snprintf(NULL, 0, "%d", offset * 32)];

            sprintf(hexOffset, "%x", offset * 32);

            string hexOffsetStr = (string) hexOffset;

            size_t remain = 64 - (hexOffsetStr.size() % 64);
            for(int i = 0; i < remain; i++) {
                zeros = zeros + "0";
            }

            strOffsets = strOffsets + zeros + hexOffsetStr;

            offset += 2;
        }
    }

    ret = contractBytes + strOffsets + ret + dynamicRet;
...
string Contract::SendTransaction(uint32_t nonceVal, uint32_t gasPriceVal, uint32_t gasLimitVal, string *toStr, string *valueStr, string *dataStr) {
    uint8_t signature[SIGNATURE_LENGTH];
    memset(signature,0,SIGNATURE_LENGTH);
    int recid[1] = {0};

    GenerateSignature(signature, recid, nonceVal, gasPriceVal, gasLimitVal, toStr, valueStr, dataStr);
    vector<uint8_t>param = RlpEncodeForRawTransaction(nonceVal, gasPriceVal, gasLimitVal, toStr, valueStr, dataStr, signature, recid[0]);
...
void Contract::GenerateSignature(uint8_t* signature, int* recid, uint32_t nonceVal, uint32_t gasPriceVal, uint32_t gasLimitVal, string* toStr, string* valueStr, string* dataStr) {
...
string Contract::GenerateBytesForString(const string* value) {
    ...
    string hexValue = Util::ConvertStringToHex(value);

    size_t remain = 64 - (hexValue.size() % 64);

    for(int i = 0; i < remain; i++){
        zeros = zeros + "0";
    }

    string res2 = hexValue + zeros;

    zeros = "";

    char hexLength[snprintf(NULL, 0, "%x", hexValue.size() / 2)];

    sprintf(hexLength, "%x", hexValue.size() / 2);

    string hexLengthStr = (string) hexLength;

    remain = 64 - (hexLengthStr.size() % 64);
    for(int i = 0; i < remain; i++) {
        zeros = zeros + "0";
    }

    string res1 = zeros + hexLengthStr;
...
vector<uint8_t> Contract::RlpEncode(uint32_t nonceVal, uint32_t gasPriceVal, uint32_t gasLimitVal, string* toStr, string* valueStr, string* dataStr) {
