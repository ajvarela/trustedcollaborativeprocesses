...
contract AbstractConsumer {
    ...
    function createDecisionObject(bytes[] memory element1, bytes[] memory element2) internal {
        // Custom attributes
        string memory patientDni = abi.decode(element2[0], (string));
        string memory vehicleId = abi.decode(element1[1], (string));
        uint256 fraudTime = abi.decode(element1[3], (uint256));

        DecisionData memory newDecisionData = DecisionData(patientDni, vehicleId, fraudTime, block.timestamp);
