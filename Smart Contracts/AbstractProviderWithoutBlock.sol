...
contract AbstractProviderWithBlock {
    ...
    function storeBusinessData(string memory _attribute1, address _attribute2, uint _attribute3, uint256 _blockTime) public {
        if (tx.origin != owner) { revert("Only the owner can store business data"); }

        allBusinessData[count].businessData["attribute1"] = abi.encode(_attribute1);
        allBusinessData[count].types["attribute1"] = "string";

        allBusinessData[count].businessData["attribute2"] = abi.encode(_attribute2);
        allBusinessData[count].types["attribute2"] = "address";

        allBusinessData[count].businessData["attribute3"] = abi.encode(_attribute3);
        allBusinessData[count].types["attribute3"] = "uint";

        allBusinessData[count].businessData["blockTime"] = abi.encode(_blockTime);
